...
class data_prefetcher():
    ...
    def preload(self):
        try:
            self.next_image, self.next_mask_image = next(self.loader)
        except StopIteration:
            self.next_image, self.next_mask_image  = None, None
            return
        with torch.cuda.stream(self.stream):
            self.next_image = self.next_image.cuda(non_blocking=True)
            self.next_mask_image = self.next_mask_image.cuda(non_blocking=True)
    ...
    def next(self):
        ...
        mask_image = self.next_mask_image
...
def train_valid(model, loss_fn, optimizer, epoch, train_valid, log_path, dataloader, print_frq=100):
    ...
    images, mask_images = prefecther.next()
    ...
    if train_valid == "train":
        ...
        # for i, images in enumerate(dataloader):
            images = (images.squeeze(0) / 255.).float()
            mask_images = (mask_images.squeeze(0) / 255.).float()
